...
from src.core.license_config import APP_VERSION
...
class MainWindow(
    ...
    _update_available_signal = pyqtSignal(dict)  # update info
    _update_check_done_signal = pyqtSignal(bool, str)  # has_update, message
    ...
    def _check_for_updates_silent(self):
        """Silent update check on startup - only shows badge if update available"""
        try:
            ...
            if has_update and update_info:
                ...
                # Use signal to update UI from main thread (thread-safe)
                self._update_check_done_signal.emit(True, "")
    ...
    def _manual_check_for_updates(self):
        """Manual update check when user clicks version button"""
        # If we already have cached update info, show it immediately
        if hasattr(self, '_cached_update_info') and self._cached_update_info:
            self._show_update_dialog(self._cached_update_info)
            return
        
        # Otherwise, check for updates
        self.version_pill.setText("Checking...")
        self.version_pill.setEnabled(False)
    ...
    def _check_for_updates_bg(self):
        ...
        try:
            ...
            if has_update and update_info:
                # Cache update info
                self._cached_update_info = update_info
                # Emit signal to show update dialog on main thread
                self._update_check_done_signal.emit(True, "")
                self._update_available_signal.emit(update_info)
            else:
                # Show "up to date" message
                self._update_check_done_signal.emit(False, f"You're running the latest version (v{APP_VERSION})")
        except Exception as e:
            # Show error
            self._update_check_done_signal.emit(False, "Could not check for updates. Please check your internet connection.")
    
    def _on_update_check_done(self, has_update, message):
        """Handle update check completion on main thread"""
        # Reset button
        self.version_pill.setText(f"v{APP_VERSION}")
        self.version_pill.setEnabled(True)
        
        # Show/hide badge
        if has_update:
            self.update_badge.setVisible(True)
            # Update tooltip when badge is shown
            if hasattr(self, '_cached_update_info') and self._cached_update_info:
                self.version_pill.setToolTip(f"Update available: v{self._cached_update_info['version']} - Click to update")
        else:
            self.update_badge.setVisible(False)
        
        # Show message if no update
        if not has_update and message:
    ...
    def _show_update_dialog(self, update_info):
        ...
        try:
            ...
            # Hide badge when showing dialog
            self.update_badge.setVisible(False)
            
            dialog = UpdateDialog(update_info, self)
            result = dialog.exec()
            
            # If user clicked "Later", show badge again
            if result == QDialog.DialogCode.Rejected and hasattr(self, '_cached_update_info'):
                self.update_badge.setVisible(True)
        except Exception as e:
            # Silently fail
            pass
    ...
    def __init__(self):
        ...
        self._update_check_done_signal.connect(self._on_update_check_done)
        self.setWindowTitle(f"Facebook Register Tool v{APP_VERSION}")
        ...
        # Check for updates on startup (silent - only shows badge if available)
        # Fixed: Now uses Qt signals for thread-safe UI updates
    ...
    def init_ui(self):
        ...
        # Version button with update check - FIRST (before expiry)
        self.version_pill = QPushButton(f"v{APP_VERSION}")
        self.version_pill.setMinimumWidth(70)
        self.version_pill.setFixedHeight(28)
        self.version_pill.setCursor(Qt.CursorShape.PointingHandCursor)
        self.version_pill.setStyleSheet("""
            QPushButton {
        ...
        """)
        self.version_pill.setToolTip("Click to check for updates manually")
        self.version_pill.clicked.connect(self._manual_check_for_updates)
        
        # Create container for version button with badge
        ...
        version_container_layout.setSpacing(0)
        version_container_layout.addWidget(self.version_pill)
        ...
        """)
        self.update_badge.setVisible(False)
        version_container_layout.addWidget(self.update_badge)
        ...
        # Action buttons - license and contact only (no version here)
    ...
    def _show_license_info(self):
        ...
        _version   = lic.get('app_version') or '1.1.2'
        try:
